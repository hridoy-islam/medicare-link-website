import { Megaphone, Code, Paintbrush, CheckCircle2 } from "lucide-react";

// =======================
// HOME PAGE CONTENT
// =======================
export const homeContent = {
  hero: {
    title: "Build Anything You Want",
    subtitle:
      "Group of Tech Enthusiasts teamed up to deliver Web Development, Apps Development, Digital Marketing, SEO, Designing and Branding your digital footprint.",
    buttonText: "Explore",
    buttonHref: "/services",
    image: "/hero-placeholder-img.jpg", // Add this image to public/
  },
  analytics: {
    pretitle: "Powerful Analytics",
    title: "Data-Driven Digital Marketing Solution",
    description:
      "With a decade of experience in the Digital industry, we proved that combining strategic thinking and expertise will create a fascinating digital experience.",
    buttonText: "Schedule a Call",
    buttonHref: "/contact",
    image: "/marketing.jpg",
  },
  whyCyberpeers: {
    pretitle: "Effortlessly Data Record",
    title: "Why Cyberpeers",
    description:
      "Cyberpeers help you achieve more building online presence, leads, sales, and revenue from our Web Design and SEO services. Helping Businesses across the UK to Rank on Page 1 of Google & Receive High-Quality Customer Inquiries.",
    buttonText: "Book a call",
    buttonHref: "/contact",
    image: "/seo.jpg",
  },
  websiteDesignGrowth: {
    title: "Complete Digital & Business Solutions for Modern Companies",
    description:
      "Are you looking for professional website design services? Are you the owner or manager of a small or medium-sized business in the UK? Do you have ambitious growth plans? These days, having a powerful and effective online presence is crucial to achieving this.",
    buttonText: "Book a Call",
    buttonHref: "/contact",
  },
  goodWebsiteFeatures: {
    title: "What makes a good website & what is quality web design?",
    image: "/web-design.jpg",
    features: [
      "Ease Of Use & Navigation",
      "Responsive AND Mobile-Friendly",
      "Website Speed",
      "Website Security",
      "Work On All Browsers",
      "SEO Optimisation",
      "Quality Content - Regularly Updated",
      "Images & Videos",
      "Aesthetics Of The Website",
      "Demonstrate Credibility & Experience",
    ],
  },
  seoAgency: {
    title:
      "Looking for an SEO agency in London that provides ethical SEO services?",
    description:
      "Do you want more clients to find and visit your website? Are you looking to generate more high-quality leads online – phone calls or emails? If so, you have come to the right place. Our SEO Services deliver results.",
    buttonText: "Book A Call",
    buttonHref: "/contact",
  },
  onSiteSeo: {
    column1: {
      title: 'What is "On-Site SEO"?',
      features: [
        "Good Website Design",
        "Site Speed",
        "Images & Videos",
        "Quality Content - Regularly Updated",
        "Website Security",
        "Optimization",
      ],
    },
    column2: {
      title: "What makes a good website & what is quality web design?",
      features: [
        "Listings & Citations",
        "Consistency",
        "Quality Backlinks",
        "Social Media Marketing",
        "Customer Reviews",
      ],
    },
  },
  topMarketingCompany: {
    title: "Grow Your Business with the Top Online Marketing Company in London",
    cards: [
      {
        title: "Experience",
        description:
          "Experience is such a thing that you can't make with money. Our years of experience of excellent data-driven results make us different from all other Web & SEO companies.",
        image: "/experience-icon.png",
      },
      {
        title: "Industry Experts",
        description:
          "We are led by biggest industry experts in United Kindom who give services to the biggest UK companies.",
        image: "/experts-icon.png",
      },
      {
        title: "Proof Of Results",
        description:
          "Every Web & SEO company says that they can deliver you results, but we don't just say but also show you the proof from our real projects.",
        image: "/results-icon.png",
      },
    ],
  },
  typesOfServices: {
    title: "Types Of Services We Provide",
    subtitle:
      "We design clean and aesthetic web applications needs so that your audience spends more time on your website.",
    services: [
      {
        title: "Web Design and Development",
        description:
          "Professional web Designing is the first step to showcase your brand and convert visitors into buyers.",
        image: "/service-01.jpg",
      },
      {
        title: "Search Engine Optimization",
        description:
          "SEO is a part of Digital marketing that help your targeted customers to find you in search engine results and take action.",
        image: "/service-02.jpg",
      },
      {
        title: "Digital Marketing",
        description:
          "Digital marketing creates opportunities for small and medium business enterprises to compete.",
        image: "/service-03.jpg",
      },
      {
        title: "Software Development",
        description:
          "Building reliable and scalable software development to meet your business goals.",
        image: "/service-04.jpg",
      },
      {
        title: "E-commerce SEO",
        description:
          "SEO has the potential to boost your e-commerce site's sales and revenue and make you the market leader.",
        image: "/service-05.jpg",
      },
      {
        title: "Social Media Marketing",
        description:
          "If you aren't sure how you should proceed in promoting your brand let us analyze your business.",
        image: "/service-06.jpg",
      },
    ],
  },
  affordableDesign: {
    title: "Affordable and Effective Website Design & SEO in London, UK",
    description:
      "Specialists in online presence building & business growth for Businesses. Our professionally branded websites rank on the 1st page of Google & generate high-quality customer leads.",
    description2:
      "We specialize in working with all Trades, including Plumbers, Heating Engineers & Bathroom Fitters, Electricians, Decorators, Conservatory Companies, Tree Surgeons & Gardeners.",
    description3:
      "Running & growing a business can be amazing & rewarding! There’s nothing like being your boss! We’d love to get your online presence working for you, so you can do what you do best!",
    buttonText: "Contact Us",
    buttonHref: "/contact",
    image: "/affordable-seo.png",
  },
  finalCta: {
    title: "Ready To Build Your Brand?",
    subtitle: "YOU ARE JUST ONE STEP AWAY TO EXPERIENCE THE PROGRESS.",
    buttonText: "Let Us Build Your Business",
    buttonHref: "/contact",
  },
  contactSection: {
    title: "Ready to get started?",
    form: {
      placeholders: {
        name: "Name",
        email: "Email",
        phone: "Phone Number",
        service: "Select A Service",
        message: "Your Message",
      },
      
      buttonText: "Submit",
    },
    info: {
      title: "Compassionate Healthcare & Home Care Services",
      description:
        "Medicare Link provides professional healthcare, home care, cleaning support, and healthcare staffing services across the UK. Our experienced caregivers deliver compassionate and personalized care tailored to every client’s needs.",
      addressTitle: "Our Address",
      address: "65 Cranbrook Road, Ilford, London, IG1 4PG, United Kingdom",
      phone: "02030111145",
      email: "info@medicarelink.co.uk",
    },
  },
};

// =======================
// ABOUT PAGE CONTENT
// =======================
export const aboutContent = {
  hero: {
    title: "About Us",
    subtitle:
      "Empowering Your Digital Realm: Crafting Inspiring Web Solutions, Unveiling Stunning Designs, and Elevating Global Business Visibility!",
  },
  companyIntro: {
    title: "The Cyberpeers Company",
    description:
      "Welcome to Cyberpeers, your digital partner for innovative solutions and creative design. Based in the heart of London, we are a dynamic force in the realm of digital marketing. From expertly crafted e-commerce solutions to bespoke custom designs that capture your brand’s essence, captivating landing pages that leave a lasting impact, and visionary graphic and logo designs.",
    buttonText: "Check Our Services",
    buttonHref: "/services",
    image: "/hero-1.jpg",
  },
  ctaStrategies: {
    pretitle: "GET CONSULTATIONS",
    title: "Strategies For Businesses Like Yours",
    items: [
      {
        heading: "Take control of your online success",
        description:
          "Stop guessing and start succeeding with our data-driven services transform your website into a success story.",
      },
      {
        heading: "Achieve your online goals with us.",
        description:
          "Don't let your website get lost in the SERP shuffle. Experience the difference of working with professionals.",
      },
    ],
    buttonText: "Contact Us",
    buttonHref: "/contact",
    image: "/about1.jpg",
  },
  teamFeatures: {
    pretitle: "Who We Are",
    title: "We Are A Professional And Experienced Team",
    items: [
      {
        icon: CheckCircle2,
        title: "Company mission",
        description:
          "Elevating businesses through innovative digital solutions and strategic marketing expertise.",
      },
      {
        icon: CheckCircle2,
        title: "Company vision",
        description:
          "Empowering businesses with cutting-edge digital solutions to foster growth, innovation, and global impact.",
      },
      {
        icon: CheckCircle2,
        title: "Best Features",
        description:
          "Empowering Brands with Innovative Web Solutions. Tailored Branding, Seamless E-commerce, Striking Visuals, and Cutting-Edge Technology.",
      },
    ],
  },
};

// =======================
// SERVICES PAGE CONTENT
// =======================
export const servicesContent = {
  hero: {
    title: "Our Services",
    subtitle:
      "We are a self-motivated digital IT enterprise specializing in web development, design, and branding your digital footprint. Our focus is on inspiring online presence and maximizing global business visibility.",
  },
  // Used for mapping to specific sections
  sections: [
    {
      id: "digital-marketing",
      iconId: "marketing",
      title: "DIGITAL MARKETING",
      description1:
        "We are a self-motivated digital IT enterprise specializing in web development, design, and branding your digital footprint.",
      description2:
        "With our services, rest assured that your brand will secure higher search engine rankings, resulting in heightened visibility. Amplifying your target audience, embrace the opportunity to enhance your brand's reach and impact.",
      lists: [
        [
          "Advanced 3A & DTM Setup Services",
          "SEO Services",
          "Local SEO Services",
          "Ecommerce SEO Services",
        ],
        [
          "Influencer Marketing",
          "Branding & Identity Marketing",
          "Mobile Marketing",
          "Lead Generation Services",
        ],
      ],
      buttonText: "Schedule a Call",
      image: "/service1.jpg",
      imagePosition: "right",
      bgColor: "light",
    },
    {
      id: "development",
      iconId: "development",
      title: "DEVELOPMENT",
      description1:
        "Welcome to our digital haven, where innovation thrives and possibilities are limitless! Delve into a world of tailored online solutions.",
      description2:
        "Discover our expertise in optimizing mobile solutions and empowering seamless content management, propelling your business towards new horizons.",
      lists: [
        ["Development Technology", "CMS Development", "Custom Development"],
        ["E-Commerce Development", "Mobile Development"],
      ],
      buttonText: "Let Us Build Your Business",
      image: "/service2.jpg",
      imagePosition: "left",
      bgColor: "dark",
    },
    {
      id: "design",
      iconId: "design",
      title: "DESIGN",
      description1:
        "Embark on a journey of digital transformation with our dynamic solutions tailored to meet your unique business needs.",
      description2:
        "From mesmerizing landing pages to visually engaging graphics and distinctive logo creations, our expertise in template design brings seamless functionality.",
      lists: [
        [
          "Graphic Design",
          "Custom Logo Design",
          "Landing Page Design",
          "CMS Theme / Template Design",
          "Marketing Collateral",
        ],
        [],
      ],
      buttonText: "Book A Call",
      image: "/service3.jpg",
      imagePosition: "right",
      bgColor: "light",
    },
  ],
  workingProcess: {
    pretitle: "Working Process",
    title: "Simple Step Follow To Complete Work",
    steps: [
      {
        title: "Step 01 Ideation Workshop",
        description:
          "Brainstorming innovative ideas between project and development Team collaboration.",
      },
      {
        title: "Step 02 Map Out",
        description:
          "Building strategy and road map of entire project agreed by both parties.",
      },
      {
        title: "Step 03 Beta Test",
        description:
          "End to end testing on a live server to ensure seamless user experience.",
      },
      {
        title: "Step 04 Rectification and Input",
        description:
          "Addressing feedback and incorporating features to meet client's specific needs.",
      },
      {
        title: "Step 05 Final Deployment",
        description:
          "Ultimate release and implementation of the completed project approved by client.",
      },
    ],
  },
  faqs: {
    pretitle: "Frequently Asked Questions",
    title: "FAQs Cyberpeers",
    image: "/service4.jpg",
    items: [
      {
        question: "What services does Cyberpeers offer?",
        answer:
          "Cyberpeers is a development and design digital agency focused on providing a wide range of services to branding and enhance business visibility worldwide. Our core services include development of e-commerce solutions, social media, email and digital marketing, custom landing pages, graphic design, logo design, and template design.",
      },
      {
        question: "How can Cyberpeers help my business grow online?",
        answer:
          "At Cyberpeers, we leverage innovative development and design strategies to elevate your brand’s online presence. Our expertise in e-commerce development ensures seamless online shopping experiences.",
      },
      {
        question: "What sets Cyberpeers apart from other digital agencies?",
        answer:
          "We take pride in our client-centric approach, delivering tailored solutions to meet your unique business requirements.",
      },
      {
        question: "Can Cyberpeers help with website maintenance and updates?",
        answer:
          "Absolutely! Our comprehensive services include website maintenance and updates to ensure your online presence remains fresh.",
      },
    ],
  },
};

// =======================
// CAREER PAGE CONTENT (UPDATED FULL DATA)
// =======================
export const careerContent = {
  hero: {
    title: "Career",
    subtitle: "Join our team and build innovative digital experiences with us.",
  },
  filters: {
    categories: [
      "All Job Category",
      "Mobile Development",
      "Design",
      "MERN",
      "Full Stack",
      "React Developer",
    ],
    types: ["All Job Type", "Remote", "Hybrid", "On-site"],
    locations: [
      "All Job Location",
      "London",
      "Remote (UK)",
      "Remote (Global)",
      "Hybrid",
    ],
  },
};

export const contactContent = {
  hero: {
    title: "Contact Us",
    subtitle:
      "Have a project in mind or a question for our team? We'd love to hear from you. Reach out and let's build something amazing together.",
  },
};
